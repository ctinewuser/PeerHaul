...
class AdminController extends Controller
 {
    ...
    public function dashboard(){
          $users = $this->user_driver->getDriverList() ;
          $delivery = $this->listing->getPendingDelivery();
          $customer = $this->user_customer->getCustomerUser();
          $active = $this->listing->getActiveDelivery();
        return view('dashboard',compact('users','customer','delivery','active'));
    }
   ...
   {  
    ...
    public function getjobBidList($id)
    {
       $jobBid = $this->bidjob->getJobBidAdminDetail($id) ;
      return view('bidjob',compact('jobBid')) ;
    }
    public function blockJob($id)
    {
        $jobBid = $this->listing->blockjob($id) ;
        return redirect()->back()
                ->with('success_msg', 'Job blcoked Succesfully!');
    }
   ...
   public function getJobDetailById($id)
   {
    //$getjobDetails = $this->job_detail->viewJobDetails($id) ;
      ...
      $getjobDetails = $this->job_detail->oldviewJobListDetails($id);
       
      return view('view-job',compact('getjobDetails','getList')) ;
   }
  
   public function viewReviewsOfDriver($id)
   {
      $allReview = $this->review->getReviewListAdmin($id) ;
      return view('review',compact('allReview')) ;
   }
