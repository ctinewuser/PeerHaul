    ...
    public function storeBid($request)
     ...
     public function getAcceptBidDetail($driver_id)
     {
      return DB::table('tbl_job_bid')
         ->where('tbl_job_bid.bid_accepted','1')
         ->get();
     ...
     /////Detail of accept bid  
      ...
      public function getCompletedBidList($driverId)
      {
        ...
        ->select( 'tbl_job_listing.id AS job_id','tbl_item_information.descriptive_title','tbl_job_listing.pick_up_location','tbl_job_listing.drop_off_location','tbl_job_listing.estimate_price')
        ->where('tbl_job_listing.job_status','=', 4)
        ->where('tbl_job_listing.driver_id', '!=', 0)
        ->get();
      }
