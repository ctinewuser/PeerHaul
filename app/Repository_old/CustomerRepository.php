...
class CustomerRepository {
    ...
    {  
        return $this->customer->where('id', $id)->update(['status' => 2]);
    ...
    public function getCustomerList(){
        return $this->customer->where('status','0')->orderBy('id','desc')->get();
    }
    public function getNotificationList()
    {
         return $this->notificationlist->get();
    }
    ...
    public function store($request){
        ...
        return $this->customer->insert($requestData);
    }

    public function customerLogin($request)
    {
        return $this->customer->where('email',$request->email)->where('password',md5($request->password))->first() ;
    }

    public function updateFCM($id,$fcm)
    {
        return $this->customer->where('id', $id)->update(['fcmToken' => $fcm]);
    }    
    ...
    public function updateCustomerProfile($request)
    {
       ...
       if($request->city !='') 
        ...
        return $this->customer->where('id',$request->customer_id)->update($req) ;
    ...
    {
        return $this->customer->where('id', $id)->update(array('password' => md5($password),'show_password'=>$password)) ;        
    } 
